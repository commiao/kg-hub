...
on_nas() {
  ...
  # `-n` 不是可选的：ssh 默认从 stdin 读并转发给远端，放在 `while read` 循环里
  # 会把剩下的输入全吞掉，循环**只跑第一次就结束**。
  #
  # 2026-09-22 实测代价：prune 拿到 3 个可删文件，只删掉了第一个，然后安静退出，
  # 还打印「已清理 1 个」—— 看起来完全正常。它在生产上第一次真跑就是这样，
  # 而 17 条单元测试全绿，因为它们从不真的发 ssh。
  #
  # 这正是「一次只做一个时藏着的缺陷，批量做就现形」（准则 29 推论）：
  # 此前每次发布的可删清单都是 0 或 1 个，这个 bug 一直没机会现形。
  #
  # 本仓唯一需要 stdin 的远端调用是 `git archive | ssh ... tar -x`，
  # 它直接用 ssh 不走 on_nas，不受影响。
  ssh -n "${SSH_OPTS[@]}" "$NAS" "$@"
}
