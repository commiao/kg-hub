...
DATA_DIR="/volume2/4T/kg-hub-data/device-liveness"
MAP_OUT="$DATA_DIR/device-liveness.json"
SNAPSHOT_OUT="$DATA_DIR/runtime/tailscale-status.json"
...
cat "$REPO/$MAP_REL" | ssh -o BatchMode=yes -o ConnectTimeout=20 "$NAS" \
  "set -eu; install -d -m 0755 '$DATA_DIR' && install -d -m 0750 '$DATA_DIR/runtime' && \
   if [ ! -f '$MAP_OUT' ]; then \
...
ssh -o BatchMode=yes -o ConnectTimeout=20 "$NAS" \
  "set -eu
   before=\$(stat -c %Y '$SNAPSHOT_OUT' 2>/dev/null || echo 0)
   deadline=\$((\$(date +%s) + 75))
   after=\$before
   while [ \"\$after\" -le \"\$before\" ] && [ \"\$(date +%s)\" -lt \"\$deadline\" ]; do
     sleep 5
     after=\$(stat -c %Y '$SNAPSHOT_OUT' 2>/dev/null || echo 0)
   done
   now=\$(date +%s)
   age=\$((now - after))
