...
- ✅ **G2 交付分层设计 + 离线验证**（step 5）：`delivery_replay` type-weighted + 探索地板；知识不变量 8/8、地板违规 0。**未接线上**（G5 才上线）。
- ✅ **G3 精确清理 apply 完成并全链路验证**（2026-07-09 关账）：软归档 27 条（26 ops_noise + INCIDENT-RETRO），total 不变、active−27、archived+27、active_ops_noise→0；server 部署带 archived 读路径过滤，canonical_context/episode_search live 不再吐归档；回滚就绪未触发。**dup 整类未动**。
- ✅ **进闸备件**：`utils/ops_noise.py` 分类器（10/10）、WS-3 接线 + `filter_replay`（全量回放 30/30 全 ops_noise、零误杀）。**未武装**（config `ops_noise.enabled=false`）。

**剩余推进顺序（定稿 2026-07-10；勿滑回"大扫除"，勿惯性打开 G4）**：

```
收 G3（已关账）
  → G5+G6-lite 机制已部署（2026-07-10，ab69e13）：
       · G5 交付分层 _tiered_rerank 接入 canonical_context pass-2 + episode_search，
         config delivery_tiering.enabled=false=INERT（实测 disabled 严格零行为变化）
       · G6-lite _log_delivery 已 LIVE → /backup/delivery-hits.jsonl 采集注入/搜索 mix 基线
  → ⏳ 下一步 = canary 微 gate：flip delivery_tiering.enabled=true + rebuild，
       用 G6-lite 前后 mix 对比验证（知识型注入占比是否上升）。需放行。
  → 再议 G4    只读侦察 + 受限 backfill（独立决策，按三闸；见下）
```
> **G6-lite 基线已见效**：`episode_search q=falkordb` 部署前 top-8 = bugfix/discovery/refactor、**0 知识型** —— 正是 flip 后期待被抬升的对照。日志格式：`{ts,endpoint,kw,tiering,picked:[{name,type}]}`。
> **为什么 G6-lite 必须傍着 G5 上**：本案最大误判来自 `usage_count` 插桩盲区（把"没插桩"当"没在用"）。G5 若上线却不补一点使用记录，后面判断"有没有发挥作用"又会靠感觉。**G5 与 G6-lite 是一对，不拆。**

### G5 flip gate（定 2026-07-10 —— 不早 flip，先攒真实 baseline）
G5 是"改排序偏好"（非 G3 那种"去坏东西"），必须看真实查询分布，否则易拿单点样本（如 `falkordb`）自证。步骤：
1. **先攒 baseline**：≥20-30 条**真实** delivery hit 或跑满一天（先到者），覆盖 `canonical_context` + `episode_search` 两面。（⚠️ 我的探针调用不算真实分布。）
2. **flip 前固化一张表**：`tools/delivery_stats.py` 按 endpoint 出 `knowledge/discovery/operational/canonical?` 占比 + 典型 query top-N，存档。
3. **flip canary**：`delivery_tiering.enabled=true` + rebuild；跑同一批 replay + 叠加真实使用一小段。
...
G4 不是收尾，是「新流量是否值得进入图」的独立决策。**摄入休眠是设计使然**（见 0.1），重开前须逐一确认三闸：
1. **入口值不值得重启**：claude-mem 自上次 watermark（图内 max obs id≈4639）后新增 obs 的**净知识价值**——是否有明显新知识，还是又回到操作日志堆积。
2. **闸是否先武装**：若重启，**WS-3 `ops_noise.enabled=true` 须先于/同批上线**，否则刚清掉的自指噪音随新流量重新进图。
3. **出口是否先变好**：**先上 G5-canary + G6-lite**（比 G4 更直接提升「发挥作用」且能度量）；G4 若做也应设计成**小批量 catch-up + compare**，**不直接恢复 15 分钟常驻摄入**。

> **⚠️ 勿信旧记忆**：kg-hub 图/旧观测里残留「launchd 每 15 分钟摄入」是**过时表述**——live 实测该 launchd job 实为 `sync_claude_mem_to_nas.sh`（只同步 db、不 ingest），摄入链路已休眠。**G4 必须以 live 侦察为准，不得按旧事实直接恢复摄入。**
> **G4 第一步 = 只读侦察（不重启）**：查 claude-mem.db 自 watermark 后新增数 / 类型分布 / 按当前 filter+ops_noise 候选配置会进多少·拒多少。画像出来再决定「先受限 backfill / 先 WS-3 flip」。**此侦察需用户显式放行才做。**
```
...
**结论：G3 完成 —— 没删、只隔离 27 条、总量不变、活跃噪音归零、`canonical_context`/`episode_search` 等 episode 级注入/搜索路径不再吐归档内容。**
> ⚠️ **仅 episode 级**：`/api/search`（edge fact）与 `search_semantic` 仍有残留（见读路径分类）——**别读成"全搜索面已彻底清洁"**。
> **封账（不是消极停手）**：G3 已关账。保留 `data/curate-G3-2026-07-09.json`（manifest，可 restore）、`backup-pre-G3-*` 快照、`health-baseline-G3pre-*.json`（验收基线）三样即可，**不再对本步追加任何生产动作**。
