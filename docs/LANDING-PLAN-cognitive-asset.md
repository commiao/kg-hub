...
**剩余（按此推进，勿滑回"大扫除"，勿惯性打开 G4）**：

```
G4  ⛔【另一类决策，非工程小尾】要不要重启 claude-mem→kg 持续摄入（让图变活）。
      不惯性打开——先按三闸重审（见下方「G4 决策框架」），第一步只 read-only 侦察、不重启。
G5  交付分层上线（出，主杠杆）：G2 设计已离线验证，把 type-weighted 接入线上排序
      （改 kg_hub_server + rebuild = 生产 gate）。比 G4 更直接提升「知识发挥作用」。
...
3. **出口是否先变好**：**倾向先上 G5**（交付分层，已离线验证，比 G4 更直接提升「发挥作用」）；G4 若做也应设计成**小批量 catch-up + compare**，**不直接恢复 15 分钟常驻摄入**。

> **G4 第一步 = 只读侦察（不重启）**：查 claude-mem.db 自 watermark 后新增数 / 类型分布 / 按当前 filter+ops_noise 候选配置会进多少·拒多少。画像出来再决定「先 G5 / 先 WS-3 flip / 开受限 backfill」。**此侦察需用户显式放行才做。**
```
