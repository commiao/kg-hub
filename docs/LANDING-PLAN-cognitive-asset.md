...
| 注入覆盖率 | 全图仅 ~1.1% 的 episode `usage_count>0`（= 曾被 canonical_context 注入过；**非**「被使用过」，见 WS-4 插桩盲区）；胶囊层 88.9% | `/dashboard/utilization` |
...
- [ ] 指标与已知事实对齐（`total_episodes ≈ 2266`、`injected_ever_rate ≈ 1.1%`）——若严重偏离说明查询口径错，需修正。
