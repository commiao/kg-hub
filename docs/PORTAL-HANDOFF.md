...
一个**报表/看板的统一入口**，把原本散落在 CLI / cron / 飞书 / `.md` 文件里的零散报表收拢到一个网址。

- 入口：`http://100.123.208.32:17171/portal`（= `/`）
- 已有一个看板：`/dashboard/capsules`（知识胶囊曝光 + 各 cwd 关键词下的实时排序与 top-3 注入；60s 自动刷新）

**它不是独立服务**，就是现有 `kg-hub-server` 这个 HTTP 服务里的几个路由。
...
- ✅ 已上线：`/portal`、`/dashboard/capsules`、`deploy/nas/redeploy.sh`、`docs/REPORTS.md`。
- 🔜 顺手能加的报表（数据已有）：
  - **使用排行** `/dashboard/usage` ← `/api/usage_ranking`（胶囊累计注入排行，最简单的练手）
  - **监控状态** `/dashboard/health` ← 容器状态/图节点数（数据见 `deploy/monitoring/`、`/api/stats`）
- 建议第一个任务：把"使用排行"做成第二张卡，完整走一遍"加报表"流程，验证 §5 + redeploy。
