...
#
# ---- 2026-09-20 决定:维持全盘最大值,不改 ----------------------------------
#
# 数据到齐了(disktemp.log 10784 条采样,跨 8 月中旬至今):
#
#     sata1   最高 61   中位 49   超 59 的采样 5.9%
#     sata2   最高 61   中位 51   超 59 的采样 0.1%
#     sata3   最高 51   中位 42   超 59 的采样 0%     ← 数据盘
#     max(全部盘) >= 59 的采样 1618 / 10784 = 15.0%
#
# 两件事同时成立:
#   1. **数据盘自己从来不热**(最高 51,离阈值 8 度)。只看它的话这道闸等于不存在。
#   2. **sata1/sata2 真的能摸到 61**,那正是 DSM 强制关机线。
#
# 所以按「保护整机」保留全盘最大值。代价是明确的、也是接受的:
# 历史上约 15% 的轮次被温度挡掉,而管线自己并不写那两块盘。
# 用「偶尔少处理一些积压」换「整台 NAS 不会被热关机」—— 2026-08 那次停了两周,
# 那个代价高出几个数量级。
#
# 这条不再是待决事项。要重新讨论,请先拿新的 disktemp.log 分布来,别凭推理。
